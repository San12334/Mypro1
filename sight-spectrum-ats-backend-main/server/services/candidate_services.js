...
class candidate_services {
  ...
  static async getCandidateDetailsByEmail(email) {
    try {
      return await fast_connection.models.candidate.findOne({is_deleted:false,email:email}).populate("created_by", { _id: 1, first_name: 1, last_name: 1});
    } catch (error) {
      throw error;
    }
  }
